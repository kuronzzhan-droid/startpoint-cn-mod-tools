...
| `version.ts.diff` | 改 `src/lib/version.ts` | `git apply`。**必须打**:上游把 CDN 最高版本缓存进模块变量且永不失效,发布新 diff 包后服务端 target 版本不推进(diff 列表有包但客户端不下载),表现为"发布了但游戏里没更新"。补丁改为按 diff 目录 mtime 失效,恢复"落包即生效、免重启" |
