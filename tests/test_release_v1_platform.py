...
class LaunchEnvironmentTests(unittest.TestCase):
    def test_child_environment_is_host_owned_exact_and_private(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            environment = LaunchEnvironment(data, cdn, modes, binding)
            base = {
...
class _FakeBackend:
    ...
    def spawn(self, command: tuple[str, ...], cwd: Path, environment: dict[str, str]) -> object:
        self.calls.append(("spawn", command, cwd, dict(environment)))
        ...
        return SimpleNamespace(
            ...
            stdout=BytesIO(self.next_stdout),
            stderr=BytesIO(self.next_stderr),
            owner=object(),
        )
    ...
    def open_process(self, pid: int) -> object | None:
        ...
        if state is None or not state.running:
            return None
        return self._new_handle(pid, "open")
...
class WindowsPlatformAdapterTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.adapter = WindowsPlatformAdapter(
            ...
            base_environment={"SECRET_TOKEN": "inherited"},
            capture_limit=32,
    ...
    def test_start_persists_exact_private_identity_and_current_process_revalidates_it(self) -> None:
        ...
        for private in (self.root, self.executable, self.server_entry, "SECRET_TOKEN", "inherited"):
    ...
    def test_server_output_is_bounded_and_released_after_owned_stop(self) -> None:
